...
void free_vma_pages(struct vma *, addr_t, __size_t);
void release_binary(void);
struct vma *find_vma_region(addr_t);
struct vma *find_vma_intersection(addr_t, addr_t);
int expand_heap(addr_t);
addr_t get_unmapped_vma_region(addr_t);
long do_mmap(struct inode *, addr_t, addr_t, unsigned int, unsigned int, addr_t, char, char, void *);
int do_munmap(addr_t, __size_t);
int do_mprotect(struct vma *, addr_t, __size_t, int);
