/*
 * fnx/include/fnx/agfs.h
 *
 * AGFS ("the ex-Be filesystem"): forked from the Be File System. The
 * on-disk structures follow the BeOS/Haiku BFS layout (block_run
 * allocation, B+tree directories, attribute indexes), but the
 * superblock magic is our own 'AGFS' — AGFS volumes no longer present
...
#define AGFS_BLOCK_SIZE		1024	/* the reference/default block size */
#define AGFS_BLOCK_SHIFT		10
...
#define AGFS_MIN_BLOCK_SIZE	1024
#define AGFS_MAX_BLOCK_SIZE	4096
#define AGFS_MIN_BLOCK_SHIFT	10
#define AGFS_MAX_BLOCK_SHIFT	12
#define AGFS_INODE_SIZE		256
#define AGFS_INODES_PER_BLOCK	(AGFS_BLOCK_SIZE / AGFS_INODE_SIZE)
#define AGFS_BLOCKS_PER_AG	8192
#define AGFS_AG_SHIFT		13
#define AGFS_NUM_DIRECT_BLOCKS	12
#define AGFS_NAME_LEN		255

/* superblock magic values (magic1 is our own 'AGFS'; magic2/magic3
...
/* inode magic + flags */
#define AGFS_INODE_MAGIC		0x3bbe0ad9
#define AGFS_INODE_IN_USE	0x00000001
#define AGFS_INODE_INLINE_DATA	0x00000080	/* file data lives in the
						 * small_data tail (X-SSD6) */
/* Haiku inode_flags (permanent bits; the low 16 bits are on-disk) */
#define AGFS_INODE_LONG_SYMLINK	0x00000040	/* target in the data stream */
#define AGFS_INODE_ATTR_INODE	0x00000004	/* legacy BeOS: attribute node */

/* X-SSD6: the largest file kept inline in the inode's small_data tail.
 * The tail is block_size - sizeof(struct agfs_inode) bytes (792 at 1K),
...
#define AGFS_INLINE_MAX		512

/* X-SSD5(a): allocation windows. When a file's stream must start a new
 * run, agfs_bmap reserves up to AGFS_DELALLOC_WINDOW contiguous blocks
 ...
 * file's committed size by agfs_write_inode (which runs at the last
...
#define AGFS_DELALLOC_WINDOW	256
...
#define AGFS_S_DOUBLE_INDEX	0x00040000	/* double index */
#define AGFS_S_ALLOW_DUPS	0x00080000	/* allow duplicates (unused) */
#define AGFS_S_LONG_LONG_INDEX	0x00200000	/* int64 index */
#define AGFS_S_ULONG_LONG_INDEX	0x00400000	/* uint64 index */
#define AGFS_S_FLOAT_INDEX	0x00800000	/* float index */
#define AGFS_S_STR_INDEX		0x01000000	/* string index */
#define AGFS_S_INT_INDEX		0x02000000	/* int32 index */
#define AGFS_S_UINT_INDEX	0x04000000	/* uint32 index */
#define AGFS_S_ATTR_DIR		0x08000000	/* attribute directory */
#define AGFS_S_ATTR		0x10000000	/* attribute */
#define AGFS_S_INDEX_DIR		0x20000000	/* index (or index directory) */
...
#define AGFS_BTREE_DUPLICATE_NODE	2
#define AGFS_BTREE_DUPLICATE_FRAGMENT	3
#define AGFS_BTREE_NUM_FRAGMENT_VALUES	7	/* values per fragment */
#define AGFS_BTREE_NUM_DUPLICATE_VALUES	125	/* values per duplicate node */
#define AGFS_BTREE_MAX_FRAGMENTS		16	/* 1024 / ((7+1)*8) */

/* inode 'type' values (attribute type of the main data stream) */
#define AGFS_FILE_TYPE_DIR	0x00000000
#define AGFS_FILE_TYPE_REG	0x00000001
#define AGFS_FILE_TYPE_LNK	0x00000002

/* B+tree */
#define AGFS_BTREE_MAGIC		0x69f6c2e8
#define AGFS_BTREE_NULL		(-1LL)
#define AGFS_BTREE_FREE		(-2LL)
#define AGFS_BTREE_STRING_TYPE	0
#define AGFS_BTREE_INT8_TYPE	1
#define AGFS_BTREE_INT16_TYPE	2
#define AGFS_BTREE_INT32_TYPE	3
#define AGFS_BTREE_UINT32_TYPE	4
#define AGFS_BTREE_INT64_TYPE	5
#define AGFS_BTREE_UINT64_TYPE	6
#define AGFS_BTREE_FLOAT_TYPE	7
#define AGFS_BTREE_DOUBLE_TYPE	8
#define AGFS_BTREE_MAX_KEY_LEN	256
...
#define AGFS_BTREE_NODE_SIZE	1024
 ...
 * stays bounded) */
#define AGFS_BTREE_MAX_PAIRS	128

/* small_data attribute types */
#define AGFS_FILE_NAME_TYPE	0x43535452	/* 'CSTR' */

/* AGFS attribute-type ioctls (FNX extension: the Linux xattr ABI has no
...
#define AGFS_ATTR_NAME_MAX	255

struct agfs_attr_info {
	char name[AGFS_ATTR_NAME_MAX + 1];
	__u32 type;			/* on GET: the record's type */
	__u64 size;			/* on GET: the value's size */
};

#define AGFS_IOC_GET_ATTR_INFO	0x42530001	/* 'BS' + 1 */
#define AGFS_IOC_SET_ATTR_TYPE	0x42530002
...
struct agfs_query {
	...
	__u32 count;
	__u32 inodes[AGFS_QUERY_MAX_RESULTS];
};
#define AGFS_QUERY_INODES_OFF	((unsigned long)&((struct agfs_query *)0)->inodes)
...
struct agfs_block_run {
	__u32 allocation_group;
	__u16 start;
	__u16 len;
} __attribute__((packed));

typedef struct agfs_block_run agfs_inode_addr;
 ...
 * mask). The volume's ag_shift is 13..15 (mkagfs 1024-byte volumes keep
 * 13; Haiku picks 13..15 by size).
 */
static __inline__ void agfs_run_encode(struct agfs_block_run *run, __blk_t block,
				      __u32 ag_shift)
{
...
struct agfs_superblock {
	...
	__u32 num_ags;
	__u32 flags;
	struct agfs_block_run log_blocks;
	__u64 log_start;
	__u64 log_end;
	__u32 magic3;
	agfs_inode_addr root_dir;
	agfs_inode_addr indices;
} __attribute__((packed));
 ...
 * offset 0 of block 0 (the boot-block sector, unused on AGFS volumes —
 * FNX boots from the ESP, and mkagfs writes no boot code). The struct
...
#define AGFS_SB_A_OFF		512
#define AGFS_SB_B_OFF		0
#define AGFS_SB_SEQ_OFF		0x84
#define AGFS_SB_CKSUM_OFF	0x8C
#define AGFS_SB_COPY_LEN	0x90

/* data stream: direct runs + indirect + double indirect + size */
struct agfs_data_stream {
	struct agfs_block_run direct[AGFS_NUM_DIRECT_BLOCKS];
	__u64 max_direct_range;
	struct agfs_block_run indirect;
	__u64 max_indirect_range;
	struct agfs_block_run double_indirect;
...
struct agfs_inode {
	...
	agfs_inode_addr parent;
	agfs_inode_addr attributes;
	__u32 type;
	__u32 inode_size;
	__u32 etc;
	union {
		struct agfs_data_stream data;
		char symlink[144];
	} u;
	 ...
	 * AGFS_BLOCK_SIZE - sizeof(struct agfs_inode) bytes) */
} __attribute__((packed));
 ...
 * stream; a leaf has overflow == AGFS_BTREE_NULL. After the header: the
...
/* fs-private superblock info (per mounted AGFS volume) */
struct agfs_sb_info {
	__u32 block_size;
	...
	/* journal (log) state */
	struct agfs_block_run log_blocks;	/* the log extent */
...
#define AGFS_LOG_MAX_BLOCKS	48	/* count <= log size - run_array block */
	__blk_t tx_blocks[AGFS_LOG_MAX_BLOCKS];	/* blocks modified in this tx */
	unsigned char *tx_data[AGFS_LOG_MAX_BLOCKS];	/* their new content */
	...
	/* X-SSD1 (TRIM): pending discard extents recorded by agfs_bfree.
...
#define AGFS_NR_PENDING_DISCARD	256
	__u32 ndiscard;
	__blk_t d_start[AGFS_NR_PENDING_DISCARD];
	__u32 d_count[AGFS_NR_PENDING_DISCARD];
};
 ...
 * tail for a given volume is block_size - sizeof(struct agfs_inode). */
#define AGFS_SMALL_DATA_SIZE	(AGFS_MAX_BLOCK_SIZE - sizeof(struct agfs_inode))

/* fs-private inode info (the raw on-disk inode, for bmap/readdir) */
#define AGFS_II_MAGIC	0x4b425349	/* 'KBSI' — marks a union holding a AGFS inode */

struct agfs_i_info {
	struct agfs_inode raw;	/* raw on-disk inode (232 bytes) */
	...
	__u32 magic;		/* AGFS_II_MAGIC while the union is ours */
};

/* block allocation (balloc.c) */
int agfs_balloc(struct superblock *);
void agfs_bfree(struct superblock *, __blk_t);
int agfs_balloc_specific(struct superblock *, __blk_t);
int agfs_balloc_contig(struct superblock *, __u32 want, __u32 *len);
void agfs_trim_prealloc(struct inode *);
void agfs_resync_used_blocks(struct superblock *);

/* inode.c */
int agfs_write_inode(struct inode *);
int agfs_ialloc(struct inode *, int);
void agfs_ifree(struct inode *);
void agfs_destroy_inode(struct inode *);
int agfs_truncate(struct inode *, __off_t);

/* journal.c */
struct buffer;	/* forward decl (fnx/buffer.h) */
__blk_t agfs_log_run_abs(struct superblock *, struct agfs_block_run *);
int agfs_log_replay(struct superblock *);
int agfs_log_begin(struct superblock *);
int agfs_log_record(struct superblock *, __blk_t, unsigned char *);
int agfs_log_commit(struct superblock *);
void agfs_log_write_block(struct superblock *, __blk_t, struct buffer *);
void agfs_log_lock(struct superblock *);
void agfs_log_unlock(struct superblock *);
...
void agfs_flush_discards(struct superblock *);
/* dual-copy superblock flush: the caller built the copy-A struct at
 * AGFS_SB_A_OFF in buf->data; stamp the sequence + checksum on both
 * copies, write the block and sync (fs/agfs/super.c) */
int agfs_sb_dual_write(struct superblock *sb, struct buffer *buf);
int agfs_sb_dual_write_nosync(struct superblock *sb, struct buffer *buf);
...
/* btree.c */
int agfs_btree_insert(struct inode *, const char *, __ino_t);
int agfs_btree_delete(struct inode *, const char *);
int agfs_btree_delete_ino(struct inode *, __ino_t);

/* ------------------------------------------------------------------ */
/* Journal (log) — faithful Haiku AGFS on-disk log-entry format (Haiku
 * fs/agfs/Journal.cpp, run_array). The log is a sequence of entries in
 ...
 */
struct agfs_run_array {
	__s32 count;		/* number of block runs in this entry */
	__s32 max_runs;		/* 127 (max run_array capacity) */
	struct agfs_block_run runs[127];	/* the modified blocks */
} __attribute__((packed));	/* 8 + 127*8 = 1024 = one block */
#define AGFS_LOG_MAX_RUNS	127

/* write (or replace) the file-name 0x13 small_data record (Haiku
 * Inode::SetName()); called at create/rename */
int agfs_inode_set_name(struct inode *, const char *);
int agfs_inode_get_name(struct inode *, char *, int);

/* the indices tree (Haiku's directory of index B+trees) - fs/bfs/indices.c */
void agfs_index_add(struct superblock *, struct inode *, const char *);
void agfs_index_remove(struct superblock *, struct inode *, const char *);
void agfs_index_resize(struct superblock *, struct inode *, __off_t, __u64);
void agfs_touch_mtime(struct inode *);
int agfs_inline_expand(struct inode *);
int agfs_file_read(struct inode *, struct fd *, char *, __size_t);
int agfs_inline_base(struct inode *);
void agfs_touch_ctime(struct inode *);
void agfs_dir_touch(struct inode *, __off_t);

/* the typed index-tree API (duplicate-key aware); 'dtype' is one of
 * the AGFS_BTREE_*_TYPE constants from the tree header */
int agfs_btree_insert_value(struct inode *, const char *, int, int, __u64);
int agfs_btree_find_value(struct inode *, const char *, int, int, __u64 *);
int agfs_btree_delete_value(struct inode *, const char *, int, int, __u64);
int agfs_btree_iterate_values(struct inode *, int,
	int (*)(const char *, int, __ino_t, void *), void *);

/* attribute inodes (Haiku's per-file attributes B+tree) - see
 * fs/agfs/attribute.c */
int agfs_attr_find(struct inode *, const char *, struct inode **);
int agfs_attr_get(struct inode *, const char *, char *, __size_t);
int agfs_attr_set(struct inode *, const char *, const char *, __size_t, __u32);
int agfs_attr_info(struct inode *, struct agfs_attr_info *);
int agfs_attr_set_type(struct inode *, const char *, __u32);
int agfs_query(struct superblock *, const char *, __u32 *, __u32);
int agfs_attr_list(struct inode *, char *, __size_t, int);
int agfs_attr_remove(struct inode *, const char *);
void agfs_attr_free_all(struct inode *);
#endif /* _FNX_BFS_H */
