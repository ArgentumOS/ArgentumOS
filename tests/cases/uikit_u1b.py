...
PROBE = "/System/Shared/tests/notification_basic"
...
class Case(BaseCase):
    ...
    def run(self, ctx):
        ...
        session.run("test -x %s && %s; echo U1B-EXIT=$?" % (PROBE, PROBE))
        out = session.output_since(mark)
        for line in out.strip().splitlines():
            if line.startswith("U1B"):
                self.note(line)

        self.check("probe-ran", "U1B-OK" in out,
                   "the notification probe ran")
        self.check("probe-exit-zero", "U1B-EXIT=0" in out,
                   "the probe exited 0")
        self.check("no-failures", "U1B-FAIL" not in out,
                   "no assertion failed")
