...
PROBE = "/System/Shared/tests/cell_basic"
...
class Case(BaseCase):
    ...
    def run(self, ctx):
        ...
        session.run("test -x %s && %s; echo U1C-EXIT=$?" % (PROBE, PROBE))
        out = session.output_since(mark)
        for line in out.strip().splitlines():
            if line.startswith("U1C"):
                self.note(line)

        self.check("probe-ran", "U1C-OK" in out,
                   "the cell probe ran")
        self.check("probe-exit-zero", "U1C-EXIT=0" in out,
                   "the probe exited 0")
        self.check("no-failures", "U1C-FAIL" not in out,
                   "no assertion failed")
