...
PROBE = "/System/Shared/tests/text_stack"
...
class Case(BaseCase):
    ...
    def run(self, ctx):
        ...
        session.run("test -x %s && %s; echo U3A-EXIT=$?" % (PROBE, PROBE))
        out = session.output_since(mark)
        for line in out.strip().splitlines():
            if line.startswith("U3A"):
                self.note(line)

        self.check("probe-ran", "U3A-OK" in out,
                   "the text-stack probe ran")
        self.check("probe-exit-zero", "U3A-EXIT=0" in out,
                   "the probe exited 0")
        self.check("no-failures", "U3A-FAIL" not in out,
                   "no assertion failed")
