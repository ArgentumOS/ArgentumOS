...
void start_dma(int channel, void *address, unsigned int count, int mode)
{
	...
	outport_b(dma_address[channel], LOW_BYTE((addr_t)address));
	outport_b(dma_address[channel], HIGH_BYTE((addr_t)address));

	/* set the physical page */
	outport_b(dma_page[channel], (addr_t)address >> 16);
