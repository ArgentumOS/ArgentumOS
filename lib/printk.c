 ...
 *	%s	string
 *
 * length modifiers (e.g: %ld or %lx or %llx)
 * --------------------------------------------------------
 *  	l	long int arguments (32-bit on i386, 64-bit on x86-64)
 *  	ll	long long int arguments (always 64-bit)
 *
 * flags (e.g: %05d or %-6s)
 * --------------------------------------------------------
			...
			switch(c = *(format)) {
				...
				case 'u':
					if(sw_l > 1) {
						...
						lunum = va_arg(args, unsigned int);
					}
					ptr_s = str;
					do {
						*(ptr_s++) = '0' + (lunum % 10);
				...
				case 'X':
					...
					ptr_s = str;
					do {
						*(ptr_s++) = (digit = (lunum & 0x0F)) > 9 ? basecase + digit - 10 : '0' + digit;
					} while(lunum /= 16);
					if(lf) {
						p = ptr_s;
					} else {
				...
				case 'p':
					...
					ptr_s = str;
					do {
						*(ptr_s++) = (digit = (lunum & 0x0F)) > 9 ? 'a' + digit - 10 : '0' + digit;
					} while(lunum /= 16);
					if(lf) {
						p = ptr_s;
					} else {
