...
`docs/gui-e-toolkit.md`. The other candidates remain documented
...
(`/dev/fb0`, mmap) — no DRM/KMS/GPU (see `docs/wayland-eval.md` §7).

## 2. The candidates

| # | Candidate | Protocol | Compositor | One-liner |
|---|---|---|---|---|
| A | **Wayland** | standard, external | custom minimal compositor | detailed in `docs/wayland-eval.md` |
...
| E′ | **E + full widget toolkit** | none (C API in libc) | system compositor + toolkit in lib | full widget set as an OS-shipped toolkit — detailed in `docs/gui-e-toolkit.md` |
| F | **TUI-only** | n/a | none | no GUI; console/ANSI (the null option) |

## 3. Candidate-by-candidate

### A. Wayland (see docs/wayland-eval.md)
