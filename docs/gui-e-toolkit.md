...
app resources live in `com.fnx.gui.conf` (and per app, e.g.
...
config domain (`com.fnx.gui.strings`) so translations can be added
...
3. **Theming / high contrast / large text via `config`**: colors, bevel
   ...
   (`com.fnx.gui.conf`, per-user scope). This is the a11y lever that
...
- **Q-L4 — Resources: config-backed.** Widget/app resources live in
  config domains (`com.fnx.gui.conf`, three scopes, dot-nested keys,
...
- **Q-L7 — UI strings: config-backed string resources (decided).**
  Strings live in `com.fnx.gui.strings` (+ per-app domains), three
