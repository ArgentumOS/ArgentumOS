...
Status: DRAFT — awaiting review. Companion to `docs/design/config-design.md`
(the `.conf` grammar, scopes, and libconfig contract) and
`docs/design/fsh-proposal.md` (the FSH layout these files live in). **Supersedes
...
- **The config machinery to build on** (config-design.md, done): flat
  ...
  per key (§5), `userland/libconfig.h` + `userland/libconfig.c`
  (typed getters, scope-explicit writes, atomic write temp+fsync+
  rename, §11), the `config` CLI (`userland/tools/config.c`), and the
...
- **No `fstab` exists.** `userland/tools/init.c` (PID 1) hardcodes
...
`userland/tools/init.c` reads this key and calls `sethostname(2)`, which
...
- `userland/tools/init.c` replaces its hardcoded `try_mount()` calls with an
...
- **`userland/tools/init.c`** — mount-table consumption (§5.5).
...
file — its line parser is untouched. `userland/tools/init.c` parses the
...
mount source is the fstype). `userland/tools/init.c`'s `mount_from_table()`
