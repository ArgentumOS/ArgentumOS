...
doctrine is settled (docs/reference/os-profile.md:34-38): one system compiler
...
the sanctioned "Clang migration" that docs/design/cpp-toolchain-plan.md §1
deferred to ("when self-hosting arrives, the wrapper swap is
`gcc`/`g++` → `clang`/`clang++` with the same ABI choices"). It also
unblocks Objective-C (GNUstep evaluation, docs/archive/gnustep-evaluation.md)
...
- **Kernel** (kernel/, kernel/boot64/, the `.S` files, the EFI link) built by
...
- One permissive roof: clang, lld, compiler-rt, libc++{,abi}, libunwind
  ...
  vendored, unlike libgcc/libstdc++ (docs/design/cpp-toolchain-plan.md:28-34).
...
- Compile kernel sources with clang using the CC64R flag set verbatim
  ...
  -fno-semantic-interposition -fvisibility=hidden -O2`) — kernel/boot64/*.S
...
- **Self-hosting**: compiling clang/lld under FNX is a later milestone
  ...
  in docs/design/self-hosting-plan.md.
...
- **Source fixes the clang kernel needed** (gcc warned where clang errors
  ...
  - `newstat/newfstat/newlstat.c`: forward-declare `fill_new_stat` (the
    ...
    builds still failed).
  - `drivers/net/virtio_net.c` + `kernel/boot64/asm64.c`: `"Nd"(port)` port-I/O
...
- **Docs**: §2 "Current state" rewritten to the M4 reality (clang
  ...
  claims in docs/design/cpp-toolchain-plan.md and docs/design/shared-libraries-plan.md
...
The FSH maps `/tmp` → `/System/Temporary Files/` (docs/design/fsh-proposal.md);
