...
Scope note: FNX's `init` is its own `userland/init.c` and its shell is
dash (`third_party/dash`) — this plan covers toybox only; the dash and
`userland/init.c` ports are tracked separately with the same mapping.
...
```c
/* lib/fnxpaths.h — FNX FSH path table (docs/fsh-proposal.md §4) */
