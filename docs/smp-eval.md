...
`docs/port-longmode-uefi.txt` lists SMP as an explicit non-goal ("for
...
- **One global IDT/GDT/TSS**: `idt[256]` (`kernel64/idt64.c:71`), one
  static `tss64` (`kernel64/gdt64.c:57`) whose RSP0 is rewritten on every
  switch via `gdt64_set_rsp0()`. FS/GS bases are GDT/MSR globals.
- **Syscall entry uses global scratch**: `syscall_entry64`
  (`kernel64/switch64.S:174-230`) saves the user RSP in the global
...
- **Memory allocator is unlocked**: single first-fit bitmap
  `page_bitmap[]` (`kernel64/mm64.c:52`), `alloc_pages64/free_pages64`
  ...
  `invalidate_tlb()` `kernel64/asm64.c:191-203`) — no cross-CPU TLB
  shootdown.
- **APIC is BSP-only and MSI-X-only**: `msix_init` enables the BSP's
  ...
  (`kernel/pic.c`, `kernel64/irq64.c:64-84`).
...
4. **Firmware tables.** Parse the ACPI **MADT** (or legacy MP table) for
   ...
   walker in the kernel (currently absent; `docs/port-longmode-uefi.txt`
...
9. **Per-CPU idle + need_resched.** `cpu_idle()` (`kernel/main.c:268`)
   ...
   `kernel64/idt64.c:731-744` reads the local one).
