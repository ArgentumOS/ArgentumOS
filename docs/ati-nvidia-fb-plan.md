...
`docs/hardware.txt`). Binding sets `video.pci_dev` (fb_init already prints
...
Update `docs/hardware.txt` (display table currently still lists the removed
bga driver) and `docs/devices.txt`; record per-family verified/unverified
