...
(be69e2e, docs/archive/gnustep-evaluation.md); candidate A — the Momo/Motif
fork — was resumed as the working toolkit direction.)

**Sequencing (decided): this plan runs AFTER the switch to dynamic
linking** (`docs/design/shared-libraries-plan.md`). M0 starts from a world in
...
- Builds target the **dynamic toolchain** (post-switch): libraries are
  ...
  versioned sonames per docs/design/shared-libraries-plan.md; apps are dynamic
...
- FNX-native desktop shell (window manager = the EMWM fork,
  docs/archive/emwm-window-manager.md; panels/file manager later) built on
  Momo, with CDE's architecture as reference
  (docs/archive/cde-fork-plan.md).
