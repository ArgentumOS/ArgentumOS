...
1. xbfs_indirect_bmap returned -EIO at the first *unset* double-indirect
   ...
   first table block, indirect.len-1 for a contiguous extension — which
...
+ S1-S8/X1-X5/J1-J6 regressions at all three sizes.
...
- `kernel/boot64/idt64.c` + `kernel/boot64/msix64.c`: IDT stubs + gates for
...
- **Mount**:
  ...
  - **Boot mount** — new `devfs_boot_mount()` called immediately after
    ...
    `open("/dev/console")` (kernel/boot64/init_trampoline64.S:31-51) resolves
    through devfs.
