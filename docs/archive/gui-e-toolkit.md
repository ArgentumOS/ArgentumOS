...
toolchain preparation for that is tracked in `docs/design/cpp-toolchain-plan.md`.
...
(`docs/eval/gui-e-native-api.md`); the wider candidate survey is
`docs/eval/gui-candidates.md`.
...
`docs/design/config-design.md`. A view reads `view_get_resource(view,
...
`docs/design/utf8-only.md` already fixes the encoding story: one encoding, one
...
## 7. Milestones (with feasibility-effort notes, per `docs/eval/gui-feasibility.md`)
...
- **P0 — Prerequisite: implement `libconfig`.** The header
  (`userland/libconfig.h`) is designed; no implementation exists, and
  Q-L4 (widget resources) depends on it. ~1–2k lines, independently
  useful.
- **M0 — API + compositor + windows** (~2–4k lines): per
  `docs/eval/gui-e-native-api.md` — compositor owns `/dev/fb0`, window tree,
...
- **M2 — Text/list** (~4–8k lines): Text, TextField, List, ComboBox,
  ...
  (consistent with `docs/design/utf8-only.md`).
...
- **M6 — Acceleration (later, by design)**: DRM/KMS page flips and a
  ...
  slots in without redesign (see `docs/eval/wayland-eval.md` §7 for the
  KMS scope).
...
- **Q-L4 — Resources: config-backed.** Widget/app resources live in
  ...
  user → shared → system precedence) per `docs/design/config-design.md` — the
