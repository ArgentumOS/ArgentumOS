...
(`docs/design/cpp-toolchain-plan.md`), which remains valid regardless.
...
This plan **depends on the C++ toolchain plan** (`docs/design/cpp-toolchain-plan.md`):
FLTK 1.4 is C++11 and FNX has no C++ yet. The libwidgets stack was removed
(`docs/archive/gui-e-toolkit.md` is marked SUPERSEDED); the compositor + libgui
...
2. **One backend compiled in**: the FNX platform (no X11/fontconfig/Cairo/
   ...
   FNX C++ wrapper (see `docs/design/cpp-toolchain-plan.md` P0–P2; same runtime
...
Land `docs/design/cpp-toolchain-plan.md` P0–P2: LLVM libc++/libc++abi/libunwind
...
1. Window chrome: v1 borderless (compositor `WINDOW_BORDER` off), with the
   ...
   design — see `docs/archive/gui-e-toolkit.md` menubar section, which is separate
