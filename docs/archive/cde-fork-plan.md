...
moved to forking **Motif (libXm) itself** — see `docs/archive/motif-fork-plan.md`.
...
1. The compositor + libgui custom window-server protocol is **dumped**;
   ...
   `/dev/fb0` (docs/design/x11-xvfb-fb-plan.md, implemented past M0/M2/M3).
...
docs/archive/gui-e-toolkit.md (E′/E′-b was "own toolkit on the compositor" — the
compositor is gone) and the candidate survey's assumptions in
docs/eval/gui-candidates.md. The toolkit question did not disappear — it is
