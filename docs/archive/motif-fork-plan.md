...
Status: **SUPERSEDED (2026-09)** by `docs/design/shrike-plan.md` — the toolkit
...
nothing vendored or implemented. Supersedes `docs/archive/cde-fork-plan.md` as
...
2. **The look — flat, modern (decided; see the Look subsection
   ...
   the `config` tool (docs/design/config-design.md; `userland/libconfig.c`);
...
- **A springs-and-struts layout container** — the OpenStep/NeXT model:
  ...
  view-model's SPRINGS/FORM instincts (docs/archive/gui-e-toolkit.md, since
...
- Consequence: the WM is the **window manager + menubar host** — the
  ...
  stay separate (see docs/archive/emwm-window-manager.md).
...
- It lands in `/System/Libraries` as a first-party shared library
  (docs/design/shared-libraries-plan.md) — one shared toolkit instance per
...
- Not a desktop-shell project (that comes later, natively — §5; its
  window manager is the EMWM fork, docs/archive/emwm-window-manager.md).
...
- No dlopen/plugin surface (docs/design/shared-libraries-plan.md §2.1).
