...
Status: **SUPERSEDED (2026-09)** by `docs/design/shrike-plan.md` §5 — the window
...
- **Lineage coherence**: EMWM is itself a Motif/libXm application, and
  ...
  the Momo toolkit forks (docs/archive/motif-fork-plan.md). The window manager
...
- Styling/identity/config work belongs to the **desktop-shell phase**
  (post-Momo, per docs/archive/momo-coding-plan.md M7+).

## 5. Known extension/conversion work (recorded, not scheduled)

- **Configuration → `.conf`**: EMWM uses `~/.emwmrc` and X resources /
  app-defaults; per the house rule (docs/archive/motif-fork-plan.md item 2,
