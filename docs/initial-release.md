...
Each app is a bundle (`docs/app-model.md`) in `/Applications`; the
...
3. **Format** — create the root filesystem (**BFS**, the only writable
...
- **An in-OS BFS formatter** — a `mkfs.bfs` tool in `/System/Tools`
...
support are removed** as part of the release. BFS is the native
writable filesystem; **FAT32 and ExFAT** drivers are planned
(ESP mount, removable media, and Disks formatting); ISO9660 remains
for read-only/install media. The kernel boots the BFS root directly
...
2. **Formatting** — create volumes with **any supported filesystem**:
   BFS (native), **FAT32**, and **ExFAT** in v1 (writable); ISO9660 is
...
format (BFS/FAT32/ExFAT), label, and mount/unmount in v1 (resize,
...
- **Opening files**: launches the right app — document types are
  resolved via app manifests (`docs/app-model.md` §3), with a fallback
...
- **Q-R7 — Disks v1 scope: as specced in §6.** GPT + MBR create/edit,
  format (BFS/FAT32/ExFAT), label, mount/unmount. Resize, extended/
