...
(`docs/gui-e-toolkit.md`) can be implemented on FNX as it stands,
...
| Widget resources | `config` utility + `libconfig.h` (designed, header written) | `docs/config-design.md`, `include/libconfig.h` |
| 3D/GPU | none — and **not needed** for v1 (fbdev-only decision, Q6/Q-L6) | `docs/wayland-eval.md` §7 |
...
1. **`libconfig` is not implemented.** `include/libconfig.h` exists;
...
3. **UTF-8 text vs Latin-9 fonts.** The shipped fonts are Latin-9 8-bit
   ...
   **deferred**, and consistent with the UTF-8-only policy
   (`docs/utf8-only.md`): v1 renders the Latin-9 subset correctly and
