...
and trusted-domain extensions this plan introduces.
...
- **D4 — Non-overridable: explicit per-domain trusted flag.** A marker
  in the system-scope file itself; `config` and libconfig honor it
  (§4). The namespace rule (`system.config.*` = always system-only) was
  considered and rejected in favor of the explicit flag so that ordinary
  `system.config.*` domains (e.g. the current Xfb domain) keep today's
  user-overridable behavior unless they opt in.
...
## 4. Trusted domains (non-overridable, D4)

### 4.1 The flag

A system-scope `.conf` file may declare, at the top:

```
trusted = true
```

`trusted` is a **reserved metadata key**: libconfig parses it, but
`config read/list` never surfaces it and domain/key validation rejects
it as a data key in any other file. It is meaningful only in a system
scope file (a `trusted = true` in Shared or user scope is ignored —
those files are data, not policy, and non-root writers could otherwise
forge it).

### 4.2 Semantics honored by libconfig and the `config` CLI

For a domain whose **system-scope** file declares `trusted = true`:

- **Reads do not fall through.** Resolution is locked to the system
  file: shared and user scope files for the same domain are ignored
  (config-design §5 resolution short-circuits after system).
- **Writes/`config delete` to the shared or user scope for the domain
  are refused** with `ACCESS` (you may not shadow a trusted domain from
  an overridable scope).
- **System-scope writes are root-only** (already the case in practice:
  `/System/Configuration` is writable by root only per the FSH) and go
  through the normal atomic writer. `trusted = true` itself can only be
  set by a root system-scope write.
- Reads of the file by **libc and other privileged consumers bypass
  libconfig entirely** (M3): they read the literal
  `/System/Configuration/<domain>.conf` path, so the trusted rule cannot
  be bypassed through a non-system configuration tree, a symlink, or a
  stale scope file.

### 4.3 What "trusted" is for

Identity (`passwd`, `group`) and the mount table are policy: a
user-scope or shared-scope override would let a non-root user alter who
may log in or what mounts at boot. `trusted = true` is the explicit,
per-file marker (D4) that makes those domains authoritative and
non-shadowable, while leaving ordinary application domains (Xfb's
`system.config.xfb`, third-party domains) on the normal overridable
resolution.

## 5. The domains

Files move from legacy names/formats to `<domain>.conf` at the same
location (`/System/Configuration/`). Names below use the first-party
`system.config.*` prefix per config-design §2/§8.
...
- **Shadow folded in** (deviation from Unix, proposed): the password
  ...
  root-only, so the separate 0600 shadow file + `x` indirection buys
  nothing and costs a second file. Marked Q2.
...
- `shells` folds into a tiny trusted or untrusted list domain (§5.3);
...
A plain (untrusted) list domain replacing the legacy `shells` line
file; `chsh` + `su` validate against it.
...
- **`third_party/musl`** — rebuilt libc (ripples to every static
  ...
  - musl needs a small **libc-internal reader**: a read-only subset of
    ...
    the fixed trusted paths (§4.2). It deliberately does not link
...
- **`third_party/toybox`** — `su`, `passwd`, `chsh`, `useradd`,
  ...
  (trusted-domain rules) and write through the atomic writer; `passwd`
...
- **`config` CLI + `userland/libconfig.c`** — group-record
  parsing/writing/enumeration (M0) + the trusted flag (M1). No change
...
### M1 — Trusted domains (libconfig + config CLI)
`trusted = true` reserved-key parsing, system-file-only semantics,
resolution lock + `ACCESS` on -u/-g writes/delete for trusted domains
(§4); tests incl. an attempted user-scope shadow being refused.
Acceptance: a trusted file cannot be shadowed from Shared or user
scope; ordinary domains keep the §5 resolution.

### M2 — Account domains ship + migrate
`system.config.passwd.conf` + `system.config.group.conf` (trusted) +
`shells` domain replace the Makefile legacy `printf`s (Makefile
...
read/write through libconfig; `passwd` writes hashes into the trusted
domain atomically.
...
### M6 — Mount table
`userland/init.c` mounts from `system.config.mounts.conf`; Q3 decision
...
- **Q1** — `trusted` also implies "read-only to non-root"? (Currently
  system writes are root-only by file permissions; the flag's write
  semantics beyond scope-locking is unspecified.)
- **Q2** — Shadow folding (hash in the record, §5.1) vs. a separate
  trusted `system.config.shadow.conf` with `x` indirection.
...
- **Q5** — `trusted` interaction with symlink domains (a `trusted`
  kernel.conf under the §12 symlink): resolve-then-read the real file's
  flag, or reject trusted symlinked domains?
