...
static void
forEachCsv(const std::string &list, const std::function<void(const std::string &)> &fn)
{
	size_t start = 0;

	while (start < list.size()) {
		size_t comma = list.find(',', start);
		std::string item = list.substr(start, comma == std::string::npos
					       ? std::string::npos
					       : comma - start);
...
std::string
interfaceEmitClassHeader(const char *className, const InterfaceClassInfo &info)
{
	std::string cls = className ? className : "";
	std::string out;

	out += "// " + cls + ".h — generated by Weaver (W4).\n";
	out += "// Edit the bodies, not the shape: Weaver regenerates this.\n";
	out += "#pragma once\n\n#include <argentum/argentum.h>\n\n";
	out += "class " + cls + " {\npublic:\n";
	out += "\t" + cls + "();\n";
	forEachCsv(info.actions, [&out](const std::string &a) {
		out += "\tvoid " + a + "();\n";
	});
	forEachCsv(info.outlets, [&out](const std::string &o) {
		out += "\targentum::View *" + o + " = nullptr;\n";
	});
	out += "};\n";
	return out;
}
...
std::string
interfaceEmitClassSource(const char *className, const InterfaceClassInfo &info)
{
	std::string cls = className ? className : "";
	std::string out;

	out += "// " + cls + ".cpp — generated by Weaver (W4).\n";
	out += "#include \"" + cls + ".h\"\n\n";
	out += cls + "::" + cls + "() {}\n";
	forEachCsv(info.actions, [&out, &cls](const std::string &a) {
		out += "\nvoid " + cls + "::" + a + "() {\n\t/* TODO */\n}\n";
	});
	out += "\n// the load-time bindings (D6): selector -> lambda\n";
	out += "void weaverBind_" + cls + "(" + cls
	     + " &obj, const char *target, argentum::InterfaceDispatcher &d)\n{\n";
	forEachCsv(info.actions, [&out](const std::string &a) {
		out += "\td.bind(target, \"" + a
		     + "\", [&obj]() { obj." + a + "(); });\n";
	});
