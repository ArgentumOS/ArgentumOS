...
class Browser;
class BrowserColumnSource;	/* browser.cpp's per-column table adapter */

/* S2.4: the column view (NSBrowser's model). A source supplies rows per
 * column; a delegate decides what a selection MEANS, so the control knows
 * nothing about files — see userland/argentum/browser.cpp. */
class BrowserSource {
public:
	virtual ~BrowserSource() {}
	virtual int browserRowCount(const Browser *b, int column) const = 0;
	virtual const char *browserRowText(const Browser *b, int column, int row) const = 0;
};
...
public:
	explicit Browser(BrowserDelegate *delegate = nullptr);
	~Browser() override;

	void setSource(BrowserSource *src);	/* one column, then reloaded */
	void addColumn();
	void truncateTo(int columns);		/* keeps 0..columns-1 */
	void reload();
	...
	int focusedColumn() const;
	void setFocusedColumn(int column);
	void selectRow(int column, int row);
	int selectedRow(int column) const;
	...
	/* browser.cpp's per-column plumbing reaches in here. BrowserColumnSource
	 * is declared at namespace scope (above), NOT here: browser.cpp defines
	 * it there, and a nested declaration would name a different type. */
	/* One column: its views and its own state. Defined here rather than
	 * forward-declared because the vector below holds it BY VALUE — a
	 * vector of an incomplete type is undefined behaviour, not a
	 * declaration trick. */
	struct Column {
		class ScrollView *scroll = nullptr;
		class TableView *table = nullptr;
		class BrowserColumnSource *adapter = nullptr;
		double w = 0;		/* pt */
		int selected = -1;
	};
	static const double dividerBandPt_;
	static const double minColumnWPt_;

	BrowserSource *source_ = nullptr;
	BrowserDelegate *delegate_ = nullptr;
	std::vector<Column> cols_;
	int focus_ = 0;
	int dragging_ = -1;
	double dragBaseW_ = 0, dragFromX_ = 0;
	double columnWidthPt_ = 180;
	class BrowserTableHost *host_ = nullptr;

	void removeColumnViews(int from);
	void layout_();
	int dividerAt(double xPt) const;
	void tableDidSelect(class TableView *t, int row);
};
