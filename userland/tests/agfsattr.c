/* agfsattr.c: OpenBFS M6 test - attributes that overflow the inode's
 ...
 * can cross-check the on-disk layout. Usage: agfsattr set|check|rm|unlink
 */
#include <stdio.h>
#include <string.h>
#include <errno.h>
...
/* the kernel's AGFS attribute-type ioctl (include/fnx/agfs.h) */
...
int main(int argc, char **argv)
{
	...
	if(argc < 2) {
		fprintf(stderr, "usage: agfsattr set|check|rm|unlink|type|mk|val\n");
		return 1;
	}

	if(!strcmp(argv[1], "type")) {
		/* agfsattr type <name> [TYPE]: query (or set) the on-disk
		 * attribute type via the AGFS ioctl (the Linux xattr ABI
		 * carries no type; Haiku's fs_stat_attr / WriteAttr do) */
		struct agfs_attr_info info;
		int fd, ok;

		if(argc < 3) {
			fprintf(stderr, "usage: agfsattr type <name> [TYPE]\n");
			return 1;
		}
		...
		strncpy(info.name, argv[2], AGFS_ATTR_NAME_MAX);
		if(argc >= 4) {
			info.type = (unsigned int)strtoul(argv[3], NULL, 0);
			r = ioctl(fd, AGFS_IOC_SET_ATTR_TYPE, &info);
			ok = (r == 0);
		...
		memset(&info, 0, sizeof(info));
		strncpy(info.name, argv[2], AGFS_ATTR_NAME_MAX);
		r = ioctl(fd, AGFS_IOC_GET_ATTR_INFO, &info);
		ok = (r == 0);
	...
	if(!strcmp(argv[1], "mk")) {
		/* agfsattr mk <name> <size>: setxattr name to 'x'*size */
	...
	if(!strcmp(argv[1], "val")) {
		/* agfsattr val <name>: getxattr and print the size */
