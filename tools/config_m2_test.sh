...
# and overridable first-party defaults (system.config.xfb) ship in
...
cp "$CFG/system.config.passwd.conf" "$ROOT/System/Configuration/"
cp "$CFG/system.config.group.conf"  "$ROOT/System/Configuration/"
cp "$CFG/system.config.shells.conf" "$ROOT/System/Configuration/"
cp "$CFG/system.config.xfb.conf"    "$ROOT/Shared/Configuration/"
...
# 2) acceptance reads
[ "$("$CONF" read system.config.passwd user.admin.uid)" = "0" ] \
	&& ok "config read system.config.passwd user.admin.uid == 0" \
	|| bad "read passwd uid"
[ "$("$CONF" read system.config.passwd user.admin.gecos)" = "Admin" ] \
	&& ok "admin record re-expresses the Admin account" \
	|| bad "admin record"
[ "$("$CONF" read system.config.group group.admin.gid)" = "0" ] \
	&& ok "group domain reads gid 0" || bad "read group gid"
[ "$("$CONF" read system.config.shells shells)" = "/System/Tools/sh" ] \
	&& ok "shells list domain reads" || bad "read shells"

# 3) Xfb default domain reads from Shared...
[ "$("$CONF" read system.config.xfb display)" = "0" ] \
	&& ok "xfb defaults resolve from Shared" \
	|| bad "xfb defaults resolve from Shared"

# 4) ...and a System copy overrides it (system-first)
printf 'display = 7\n' > "$ROOT/System/Configuration/system.config.xfb.conf"
[ "$("$CONF" read system.config.xfb display)" = "7" ] \
...
"$CONF" read -s system.config.passwd > "$ROOT/pw.before"
"$CONF" write -s system.config.passwd user.admin.uid 0 >/dev/null
"$CONF" read -s system.config.passwd > "$ROOT/pw.after"
cmp -s "$ROOT/pw.before" "$ROOT/pw.after" \
	&& ok "passwd domain round-trips (key set stable)" \
	|| bad "passwd domain round-trips"
"$CONF" read -g system.config.xfb > "$ROOT/xfb.before"
"$CONF" write -g system.config.xfb display "0" >/dev/null
"$CONF" read -g system.config.xfb > "$ROOT/xfb.after"
