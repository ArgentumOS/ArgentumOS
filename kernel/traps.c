...
const char *elf_lookup_symbol(unsigned int addr)
{
	...
	vsymtab = (Elf32_Shdr *)P2V((addr_t)symtab);
	vstrtab = (Elf32_Shdr *)P2V((addr_t)strtab);
...
void stack_backtrace(void)
{
	...
	sp += ((sizeof(struct sigcontext) / sizeof(unsigned int)) - 5) * sizeof(unsigned int);
	p = (unsigned int *)P2V(sp);
	for(n = 1; n <= 32; n++) {
		printk(" %08x", *p);
		p++;
		if(!(n % 8)) {
...
int dump_registers(unsigned int trap, struct sigcontext *sc)
{
	...
	if(trap == 14) {	/* Page Fault */
		GET_CR2(cr2);
		printk("%s at 0x%lx (%s) with error code 0x%02x%s", traps_table[trap].name, cr2, sc->err & PFAULT_W ? "writing" : "reading", sc->err, sc->err & PAGE_USER ? "\n" : " in kernel mode.\n");
	} else {
