...
int data_proc_pid_cmdline(char *buffer, __pid_t pid)
{
	...
	size = 0;
	if((p = get_proc_by_pid(pid))) {
		offset = (addr_t)p->argv & ~PAGE_MASK;
		addr = get_mapped_addr(p, (addr_t)p->argv) & PAGE_MASK;
		addr = P2V(addr);
		argv = (char **)(addr + offset);
		for(n = 0; n < p->argc && (addr_t)argv[n]; n++) {
			offset = (addr_t)argv[n] & ~PAGE_MASK;
			addr = get_mapped_addr(p, (addr_t)argv[n]) & PAGE_MASK;
			addr = P2V(addr);
...
int data_proc_pid_environ(char *buffer, __pid_t pid)
{
	...
	size = 0;
	if((p = get_proc_by_pid(pid))) {
		offset = (addr_t)p->envp & ~PAGE_MASK;
		addr = get_mapped_addr(p, (addr_t)p->envp) & PAGE_MASK;
		addr = P2V(addr);
		envp = (char **)(addr + offset);
		for(n = 0; n < p->envc && (addr_t)envp[n]; n++) {
			offset = (addr_t)envp[n] & ~PAGE_MASK;
			addr = get_mapped_addr(p, (addr_t)envp[n]) & PAGE_MASK;
			addr = P2V(addr);
