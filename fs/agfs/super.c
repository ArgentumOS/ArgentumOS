 ...
 * fnx/fs/agfs/super.c
 *
 * AGFS (BeOS) filesystem: superblock, bitmap cache, registration, mount.
 *
...
/* file operations */
extern int agfs_open(struct inode *, struct fd *);
extern int agfs_close(struct inode *, struct fd *);
extern int file_read(struct inode *, struct fd *, char *, __size_t);
extern int agfs_file_write(struct inode *, struct fd *, const char *, __size_t);
extern int agfs_ioctl(struct inode *, struct fd *, int, addr_t);
extern __loff_t agfs_file_llseek(struct inode *, __loff_t);
extern int agfs_dir_read(struct inode *, struct fd *, char *, __size_t);
extern int agfs_readdir(struct inode *, struct fd *, struct dirent *, __size_t);
extern int agfs_readdir64(struct inode *, struct fd *, struct dirent64 *, __size_t);

/* inode operations */
extern int agfs_read_inode(struct inode *);
extern int agfs_write_inode(struct inode *);
extern int agfs_bmap(struct inode *, __off_t, int);
extern int agfs_lookup(const char *, struct inode *, struct inode **);
extern int agfs_truncate(struct inode *, __off_t);
extern int agfs_create(struct inode *, char *, int, __mode_t, struct inode **);
extern int agfs_mkdir(struct inode *, char *, __mode_t);
extern int agfs_mknod(struct inode *, char *, __mode_t, __dev_t);
extern int agfs_link(struct inode *, struct inode *, char *);
extern int agfs_unlink(struct inode *, struct inode *, char *);
extern int agfs_rmdir(struct inode *, struct inode *);
extern int agfs_rename(struct inode *, struct inode *, struct inode *, struct inode *, char *, char *);
extern int agfs_symlink(struct inode *, char *, char *);
extern int agfs_readlink(struct inode *, char *, __size_t);
extern int agfs_followlink(struct inode *, struct inode *, struct inode **);
extern int agfs_ialloc(struct inode *, int);
extern void agfs_ifree(struct inode *);
extern int agfs_getxattr(struct inode *, const char *, char *, __size_t);
extern int agfs_setxattr(struct inode *, const char *, const char *, __size_t, int);
extern int agfs_listxattr(struct inode *, char *, __size_t);
extern int agfs_removexattr(struct inode *, const char *);

/* superblock operations */
static void agfs_statfs(struct superblock *, struct statfs *);
static void agfs_release_superblock(struct superblock *);
static int agfs_write_superblock(struct superblock *);
static int agfs_read_superblock(__dev_t, struct superblock *);

struct fs_operations agfs_fsop = {
	FSOP_REQUIRES_DEV,
	0,

	agfs_open,		/* open */
	agfs_close,		/* close */
	agfs_file_read,		/* read (inline files; else generic) */
	agfs_file_write,		/* write */
	agfs_ioctl,		/* ioctl */
	agfs_file_llseek,	/* llseek */
	agfs_readdir,		/* readdir */
	agfs_readdir64,		/* readdir64 */
	NULL,			/* mmap */
	NULL,			/* select */

	agfs_readlink,		/* readlink */
	agfs_followlink,		/* followlink */
	agfs_bmap,		/* bmap */
	agfs_lookup,		/* lookup */
	agfs_rmdir,		/* rmdir */
	agfs_link,		/* link */
	agfs_unlink,		/* unlink */
	agfs_symlink,		/* symlink */
	agfs_mkdir,		/* mkdir */
	agfs_mknod,		/* mknod */
	agfs_truncate,		/* truncate */
	agfs_create,		/* create */
	agfs_rename,		/* rename */
	...
	agfs_read_inode,		/* read_inode */
	agfs_write_inode,	/* write_inode */
	agfs_ialloc,		/* ialloc */
	agfs_ifree,		/* ifree */
	agfs_statfs,		/* statfs */
	agfs_read_superblock,	/* read_superblock */
	NULL,			/* remount_fs */
	agfs_write_superblock,	/* write_superblock */
	agfs_release_superblock,	/* release_superblock */

	agfs_getxattr,		/* getxattr */
	agfs_setxattr,		/* setxattr */
	agfs_listxattr,		/* listxattr */
	agfs_removexattr,	/* removexattr */
	agfs_destroy_inode,	/* destroy_inode (kept last) */
};

static void agfs_statfs(struct superblock *sb, struct statfs *buf)
{
	buf->f_type = AGFS_SUPER_MAGIC1;
	buf->f_bsize = sb->s_blocksize;
	buf->f_blocks = sb->u.agfs.num_blocks;
	buf->f_bfree = sb->u.agfs.num_blocks - sb->u.agfs.used_blocks;
	buf->f_bavail = buf->f_bfree;
	/* inodes are ordinary data blocks in AGFS; report the capacity and
	 * the remaining free blocks as the inode counts */
	buf->f_files = sb->u.agfs.num_blocks;
	buf->f_ffree = buf->f_bfree;
	buf->f_namelen = AGFS_NAME_LEN;
}

static void agfs_free_bitmap(struct superblock *sb)
{
	__u32 i;
	...
	for(i = 0; i < sb->u.agfs.bitmap_chunks; i++) {
		if(sb->u.agfs.bitmap[i]) {
			kfree((addr_t)sb->u.agfs.bitmap[i]);
		}
	}
	kfree((addr_t)sb->u.agfs.bitmap);
	sb->u.agfs.bitmap = NULL;
}

static void agfs_release_superblock(struct superblock *sb)
{
	agfs_unreg_sb(sb);
	struct buffer *buf;
	__u64 i;
	 ...
	 * agfs_write_superblock drains it (the VFS flushes more inodes
	 * after release_superblock) */
	sb->u.agfs.log_draining = 1;
 ...
 */
static int agfs_write_superblock(struct superblock *sb)
{
	struct buffer *buf;
	...
	agfs_log_lock(sb);
	for(l = 0; l < sb->u.agfs.log_blocks.len; l++) {
		if((buf = bread(sb->dev,
				agfs_log_run_abs(sb, &sb->u.agfs.log_blocks) + l,
	...
	bsb = (struct agfs_superblock *)(buf->data + 512);
	...
	memset_b(bsb, 0, 0x84);
	/* preserve the on-disk volume name (mkagfs volumes keep "AGFS",
	...
	bsb->magic1 = AGFS_SUPER_MAGIC1;
	bsb->fs_byte_order = AGFS_SUPER_BYTEORDER;
	bsb->block_size = sb->u.agfs.block_size;
	bsb->block_shift = sb->s_blocksize_bits;
	bsb->num_blocks = sb->u.agfs.num_blocks;
	bsb->used_blocks = sb->u.agfs.used_blocks;
	bsb->inode_size = sb->s_blocksize;
	bsb->magic2 = AGFS_SUPER_MAGIC2;
	bsb->blocks_per_ag = sb->u.agfs.blocks_per_ag;
	bsb->ag_shift = sb->u.agfs.ag_shift;
	bsb->num_ags = sb->u.agfs.num_ags;
	...
	agfs_run_encode(&bsb->root_dir, sb->u.agfs.root_inode, sb->u.agfs.ag_shift);
	bsb->root_dir.len = 1;
	...
	if(sb->u.agfs.indices_inode) {
		agfs_run_encode(&bsb->indices, sb->u.agfs.indices_inode,
			       sb->u.agfs.ag_shift);
		bsb->indices.len = 1;
	} else {
 ...
 * AGFS_SB_A_OFF in buf->data; stamp both copies with the same new
 ...
 * (agfs_read_superblock) takes the valid copy with the highest
 * sequence. */
 ...
 * dirty in the cache). */
static void agfs_sb_dual_stamp(struct superblock *sb, struct buffer *buf)
{
	unsigned char *a, *b;
	__u64 seq;
	__u32 cksum;
	...
	a = buf->data + AGFS_SB_A_OFF;
	b = buf->data + AGFS_SB_B_OFF;
	seq = ++sb->u.agfs.sb_seq;
	/* the struct is identical in both copies; re-stamp the tail */
	memcpy_b(b, a, 512);
	memset_b(a + AGFS_SB_SEQ_OFF, 0, 512 - AGFS_SB_SEQ_OFF);
	memset_b(b + AGFS_SB_SEQ_OFF, 0, 512 - AGFS_SB_SEQ_OFF);
	cksum = 0;
	for(j = 0; j < AGFS_SB_SEQ_OFF; j += 4) {
		cksum += *(const __u32 *)(a + j);
	}
	*(__u64 *)(a + AGFS_SB_SEQ_OFF) = seq;
	*(__u64 *)(b + AGFS_SB_SEQ_OFF) = seq;
	*(__u32 *)(a + AGFS_SB_CKSUM_OFF) = cksum;
	*(__u32 *)(b + AGFS_SB_CKSUM_OFF) = cksum;
	bwrite(buf);
}

int agfs_sb_dual_write(struct superblock *sb, struct buffer *buf)
{
	agfs_sb_dual_stamp(sb, buf);
	sync_buffers(sb->dev);
	return 0;
}
...
static int agfs_read_superblock(__dev_t dev, struct superblock *sb)
{
	struct buffer *buf;
	struct agfs_superblock *bsb;
	struct agfs_block_run *root;
	__u32 root_block;
	__u32 i;
	...
	if(!(buf = bread(dev, 0, AGFS_BLOCK_SIZE))) {
	 ...
	 * (copy A @512, copy B @0 = the boot sector, free on AGFS
	...
	bsb = (struct agfs_superblock *)(buf->data + AGFS_SB_A_OFF);
	{
		__u32 best_seq = 0, i;
		int best = -1;
		struct agfs_superblock *cand;
		for(i = 0; i < 2; i++) {
			unsigned char *area = buf->data +
				(i ? AGFS_SB_B_OFF : AGFS_SB_A_OFF);
			__u32 cksum = 0, stored;
			int j;
			cand = (struct agfs_superblock *)area;
			if(cand->magic1 != AGFS_SUPER_MAGIC1 ||
			   cand->magic2 != AGFS_SUPER_MAGIC2 ||
			   cand->magic3 != AGFS_SUPER_MAGIC3) {
				continue;
			}
			for(j = 0; j < AGFS_SB_SEQ_OFF; j += 4) {
				cksum += *(const __u32 *)(area + j);
			}
			stored = *(const __u32 *)(area + AGFS_SB_CKSUM_OFF);
			if(*(const __u64 *)(area + AGFS_SB_SEQ_OFF) == 0) {
			...
			if(*(const __u64 *)(area + AGFS_SB_SEQ_OFF) > best_seq) {
				best_seq = *(const __u64 *)(area + AGFS_SB_SEQ_OFF);
				best = i;
			}
		}
		if(best < 0) {
		...
		if(best == 1 && best_seq > 0) {
			printk("AGFS: superblock copy B (block 0, seq %lu) recovered "
			       "a torn copy A.\n", best_seq);
		}
		bsb = (struct agfs_superblock *)(buf->data + AGFS_SB_A_OFF);
		if(best == 1) {
			...
			memcpy_b(buf->data + AGFS_SB_A_OFF, buf->data + AGFS_SB_B_OFF,
				 512);
			bsb = (struct agfs_superblock *)(buf->data + AGFS_SB_A_OFF);
		}
		sb->u.agfs.sb_seq = best_seq;
	}

	if(bsb->block_size < AGFS_MIN_BLOCK_SIZE
	   || bsb->block_size > AGFS_MAX_BLOCK_SIZE
	   || (bsb->block_size & (bsb->block_size - 1))) {
		printk("WARNING: %s(): unsupported AGFS block size %d.\n",
	...
	sb->dev = dev;
	sb->fsop = &agfs_fsop;
	sb->s_blocksize_bits = AGFS_MIN_BLOCK_SHIFT;
	while((1u << sb->s_blocksize_bits) < bsb->block_size) {
		sb->s_blocksize_bits++;
	}
	sb->s_blocksize = bsb->block_size;
	sb->u.agfs.block_size = bsb->block_size;
	sb->u.agfs.blocks_per_ag = bsb->blocks_per_ag;
	sb->u.agfs.ag_shift = bsb->ag_shift;
	sb->u.agfs.num_ags = bsb->num_ags;
	sb->u.agfs.num_blocks = bsb->num_blocks;
	sb->u.agfs.used_blocks = bsb->used_blocks;
	sb->u.agfs.flags = bsb->flags;
	sb->u.agfs.next_free = 0;

	root = &bsb->root_dir;
	root_block = (root->allocation_group << bsb->ag_shift) + root->start;
	sb->u.agfs.root_inode = root_block;

	/* the indices directory (0 when the volume has none) */
	if(bsb->indices.len) {
		sb->u.agfs.indices_inode = (bsb->indices.allocation_group
			<< bsb->ag_shift) + bsb->indices.start;
	} else {
		sb->u.agfs.indices_inode = 0;
	}

	/* journal (log) state: the extent + positions come from the disk */
	sb->u.agfs.log_blocks = bsb->log_blocks;
	sb->u.agfs.log_start = bsb->log_start;
	sb->u.agfs.log_end = bsb->log_end;
	sb->u.agfs.tx_depth = 0;
	sb->u.agfs.tx_nblocks = 0;
	sb->u.agfs.journal_locked = 0;
	sb->u.agfs.journal_wanted = 0;

	/* load the bitmap into memory (chunked: a large volume's bitmap is
	 * many times the kmalloc cap of one page) */
	sb->u.agfs.bitmap_blocks = sb->u.agfs.num_ags * sb->u.agfs.blocks_per_ag;
	sb->u.agfs.bitmap_chunks =
		((sb->u.agfs.bitmap_blocks * sb->u.agfs.block_size)
		 + PAGE_SIZE - 1) / PAGE_SIZE;
	if(!(sb->u.agfs.bitmap = (unsigned char **)kmalloc(
			sb->u.agfs.bitmap_chunks * sizeof(unsigned char *)))) {
	...
	for(i = 0; i < (int)sb->u.agfs.bitmap_chunks; i++) {
		sb->u.agfs.bitmap[i] = NULL;
	}
	for(i = 0; i < (int)sb->u.agfs.bitmap_blocks; i++) {
		struct buffer *bb;
		__u32 chunk = (i * sb->u.agfs.block_size) >> 12;
		__u32 coff = (i * sb->u.agfs.block_size) & (PAGE_SIZE - 1);
		if(!sb->u.agfs.bitmap[chunk]) {
			if(!(sb->u.agfs.bitmap[chunk] =
			...
			memset_b(sb->u.agfs.bitmap[chunk], 0, PAGE_SIZE);
		}
		if(!(bb = bread(dev, 1 + i, sb->u.agfs.block_size))) {
			superblock_unlock(sb);
			brelse(buf);
			return -EIO;
		}
		memcpy_b(sb->u.agfs.bitmap[chunk] + coff,
			bb->data, sb->u.agfs.block_size);
		brelse(bb);
	}
...
int agfs_init(void)
{
	return register_filesystem("agfs", &agfs_fsop);
}
