/*
 * fnx/fs/agfs/query.c - the volume query engine (Haiku's BQuery).
 *
 * Linux's ABI has no BeOS fs_query syscall, so the query surface is a
 * AGFS ioctl (AGFS_IOC_QUERY) + a userland tool. The engine follows
...
extern int agfs_btree_iterate_values(struct inode *, int,
...
struct agfs_qnode {
	...
	char attr[AGFS_BTREE_MAX_KEY_LEN];
	int vlen;
	char value[AGFS_BTREE_MAX_KEY_LEN];	/* raw value text */
	struct agfs_qnode *l, *r;		/* BQ_AND / BQ_OR children */
};
...
#define AGFS_QCHUNK_CAP	1020	/* inos[1020] is the overflow slot;
				 * 1021*4 + 4 + 8 = 4096 == PAGE_SIZE */

struct agfs_qchunk {
	__u32 inos[AGFS_QCHUNK_CAP + 1];	/* inos[CAP] = overflow slot */
	__u32 count;
	struct agfs_qchunk *next;
};

struct agfs_qset {
	struct agfs_qchunk *head, *tail;
	__u32 count;			/* total entries across all chunks */
};

static void agfs_qset_free(struct agfs_qset *s)
{
	struct agfs_qchunk *c = s->head;

	while(c) {
		struct agfs_qchunk *n = c->next;
...
static struct agfs_qchunk *agfs_qchunk_new(void)
{
	struct agfs_qchunk *c;

	if(!(c = (struct agfs_qchunk *)kmalloc(sizeof(struct agfs_qchunk)))) {
		return NULL;
	}
	c->count = 0;
	c->next = NULL;
...
static __u32 agfs_qchunk_bsearch(const struct agfs_qchunk *c, __u32 ino)
{
...
static int agfs_qchunk_insert(struct agfs_qset *s, struct agfs_qchunk *c,
			     __u32 off, __u32 ino)
{
	for(;;) {
		__u32 tailv;
		...
		c->count++;
		s->count++;
		if(c->count <= AGFS_QCHUNK_CAP) {
			return 0;
		}
		/* overflow: the last element falls into the next chunk */
		tailv = c->inos[AGFS_QCHUNK_CAP];
		c->count = AGFS_QCHUNK_CAP;
		s->count--;
		if(!c->next) {
			struct agfs_qchunk *n;

			if(!(n = agfs_qchunk_new())) {
				return -ENOMEM;
			}
			c->next = n;
			s->tail = n;
...
static int agfs_qset_add(struct agfs_qset *s, __u32 ino)
{
	struct agfs_qchunk *c;
	__u32 off;
	...
	if(c->inos[c->count - 1] < ino) {
		...
		off = agfs_qchunk_bsearch(c, ino);
		if(off < c->count && c->inos[off] == ino) {
			return 0;	/* already there */
		}
	}
	return agfs_qchunk_insert(s, c, off, ino);
}

/* s1 AND s2 -> s1 (in place; the result fits in s1's own chunks) */
static void agfs_qset_and(struct agfs_qset *s1, struct agfs_qset *s2)
{
	struct agfs_qchunk *c1 = s1->head, *c2 = s2->head;
	struct agfs_qchunk *w = s1->head, *lastw = NULL;
	...
	if(!lastw) {
		agfs_qset_free(s1);	/* empty result */
		return;
	}
	lastw->count = wi;
	{
		struct agfs_qchunk *c = lastw->next;

		while(c) {
			struct agfs_qchunk *nxt = c->next;
...
static int agfs_qset_or(struct agfs_qset *s1, struct agfs_qset *s2,
		       struct agfs_qset *out)
{
	struct agfs_qchunk *c1 = s1->head, *c2 = s2->head, *tail = NULL;
	__u32 i = 0, j = 0, wi = 0;
...
static __u64 agfs_q_strtou64(const char *s, int base)
{
	__u64 value = 0;
...
static __s64 agfs_q_strto64(const char *s, int base)
{
	int neg = 0;
	...
	{
		__u64 u = agfs_q_strtou64(s, base);
 ...
 * the arithmetic) */
static void agfs_q_strtod(const char *s, double *out)
{
...
static int agfs_qtype_size(int dtype)
{
	switch(dtype) {
...
static int agfs_q_parse_value(int dtype, const char *v, int vlen, char *out)
{
	char tmp[64];
	...
	switch(dtype) {
	case AGFS_BTREE_INT8_TYPE: {
		__s8 x = (__s8)agfs_q_strto64(tmp, 0);
		memcpy_b(out, &x, 1);
		return 1;
	}
	case AGFS_BTREE_INT16_TYPE: {
		__s16 x = (__s16)agfs_q_strto64(tmp, 0);
		memcpy_b(out, &x, 2);
		return 2;
	}
	case AGFS_BTREE_INT32_TYPE: {
		__s32 x = (__s32)agfs_q_strto64(tmp, 0);
		memcpy_b(out, &x, 4);
		return 4;
	}
	case AGFS_BTREE_UINT32_TYPE: {
		__u32 x = (__u32)agfs_q_strtou64(tmp, 0);
		memcpy_b(out, &x, 4);
		return 4;
	}
	case AGFS_BTREE_INT64_TYPE: {
		__s64 x = agfs_q_strto64(tmp, 0);
		memcpy_b(out, &x, 8);
		return 8;
	}
	case AGFS_BTREE_UINT64_TYPE: {
		__u64 x = agfs_q_strtou64(tmp, 0);
		memcpy_b(out, &x, 8);
		return 8;
	}
	case AGFS_BTREE_FLOAT_TYPE: {
...
static int agfs_q_class_match(const char **pp, char c)
{
	const char *p = *pp;
...
static int agfs_q_pattern_match(const char *pat, const char *s)
{
	for(;;) {
		switch(*pat) {
		case 0:
		...
		case '[': {
			...
			if((m = agfs_q_class_match(&pat, *s)) < 0) {
				return 0;
			}
			if(!m) {
				return 0;
...
static int agfs_q_is_pattern(const char *v, int vlen)
{
	int i;
...
struct agfs_q_ctx {
	struct agfs_qnode *node;
	int dtype;
	char key[AGFS_BTREE_MAX_KEY_LEN];
	int keylen;
	int use_pattern;
	struct agfs_qset set;
};

/* does the stored key satisfy (op, value)? */
static int agfs_q_key_matches(struct agfs_q_ctx *c, const char *key, int keylen)
{
	struct agfs_qnode *n = c->node;
	int cmp;

	if(c->dtype == AGFS_BTREE_STRING_TYPE) {
		if((n->op == BQ_OP_EQUAL || n->op == BQ_OP_NOT_EQUAL)
		   && c->use_pattern) {
			char v[AGFS_BTREE_MAX_KEY_LEN + 1];
			int m;

			memcpy_b(v, n->value, n->vlen);
			v[n->vlen] = 0;
			{
				char k[AGFS_BTREE_MAX_KEY_LEN + 1];
				memcpy_b(k, key, keylen);
				k[keylen] = 0;
				m = agfs_q_pattern_match(v, k);
			}
		...
		__s64 s1 = 0, s2 = 0;
		__u64 u1 = 0, u2 = 0;
		int sz = agfs_qtype_size(c->dtype);
...
static int agfs_q_collect_cb(const char *key, int keylen, __ino_t ino, void *arg)
{
	struct agfs_q_ctx *c = (struct agfs_q_ctx *)arg;

	if(agfs_q_key_matches(c, key, keylen)) {
		return agfs_qset_add(&c->set, (__u32)ino);
	}
	return 0;
}
...
static int agfs_q_index_dtype(struct inode *idx)
{
	__u32 m = idx->u.agfs.raw.mode;

	if(m & AGFS_S_STR_INDEX) {
		return AGFS_BTREE_STRING_TYPE;
	}
	if(m & AGFS_S_INT_INDEX) {
		return AGFS_BTREE_INT32_TYPE;
	}
	if(m & AGFS_S_UINT_INDEX) {
		return AGFS_BTREE_UINT32_TYPE;
	}
	if(m & AGFS_S_LONG_LONG_INDEX) {
		return AGFS_BTREE_INT64_TYPE;
	}
	if(m & AGFS_S_ULONG_LONG_INDEX) {
		return AGFS_BTREE_UINT64_TYPE;
	}
	if(m & AGFS_S_FLOAT_INDEX) {
		return AGFS_BTREE_FLOAT_TYPE;
	}
	if(m & AGFS_S_DOUBLE_INDEX) {
		return AGFS_BTREE_DOUBLE_TYPE;
	}
	return -EINVAL;
}

/* resolve 'attr' to its index file inode (iget'd) */
static struct inode *agfs_q_find_index(struct superblock *sb, const char *attr,
				      int *dtype)
{
	...
	if(!(dir = iget(sb, sb->u.agfs.indices_inode))) {
		return NULL;
	}
	ino = 0;
	agfs_btree_find(dir, attr, &ino);
	iput(dir);
	if(!ino) {
		return NULL;
	}
	...
	if((*dtype = agfs_q_index_dtype(idx)) < 0) {
		iput(idx);
		return NULL;
	}
	return idx;
 ...
 */
static int agfs_q_eval_formula(struct superblock *sb, struct agfs_qnode *n,
			      struct agfs_qset *out)
{
	struct inode *idx;
	struct agfs_q_ctx c;
	...
	c.node = n;
	c.dtype = AGFS_BTREE_STRING_TYPE;
	/* c.set is zeroed by memset_b; chunks allocate on demand */
	res = 0;

	idx = agfs_q_find_index(sb, n->attr, &c.dtype);
	if(!idx) {
		agfs_qset_free(&c.set);
		*out = c.set;
	...
	if(c.dtype == AGFS_BTREE_STRING_TYPE) {
		...
		c.use_pattern = (n->op == BQ_OP_EQUAL || n->op == BQ_OP_NOT_EQUAL)
			&& agfs_q_is_pattern(n->value, n->vlen);
	} else {
		if((c.keylen = agfs_q_parse_value(c.dtype, n->value, n->vlen,
						 c.key)) < 0) {
			iput(idx);
			agfs_qset_free(&c.set);
			return -EINVAL;
	...
	res = agfs_btree_iterate_values(idx, c.dtype, agfs_q_collect_cb, &c);
	iput(idx);
	if(res < 0) {
		agfs_qset_free(&c.set);
		return res;
...
static const char *agfs_q_skip(const char *p)
{
	while(*p == ' ' || *p == '\t' || *p == '\n') {
		p++;
	}
...
static int agfs_q_negate(struct agfs_qnode *n);

static struct agfs_qnode *agfs_q_new(int type)
{
	struct agfs_qnode *n;

	if(!(n = (struct agfs_qnode *)kmalloc(sizeof(struct agfs_qnode)))) {
		return NULL;
	}
...
static int agfs_q_negate(struct agfs_qnode *n)
{
	...
	if((agfs_q_negate(n->l)) < 0) {
		return -ENOMEM;
	}
	if((agfs_q_negate(n->r)) < 0) {
		return -ENOMEM;
	}
	t = n->l;
	n->l = n->r;
...
static void agfs_q_free(struct agfs_qnode *n)
{
	if(!n) {
		return;
	}
...
static struct agfs_qnode *agfs_q_parse_expr(const char **pp, int *err, int depth);

/* equation: attr op value */
static struct agfs_qnode *agfs_q_parse_formula(const char **pp, int *err)
{
	const char *p = *pp;
	struct agfs_qnode *n;
	char *q;
	...
	while(*p && *p != '=' && *p != '!' && *p != '>' && *p != '<'
	      && *p != ' ' && *p != '\t' && *p != '(' && *p != ')' ) {
		if(q - n->attr >= AGFS_BTREE_MAX_KEY_LEN - 1) {
			*err = -EINVAL;
			agfs_q_free(n);
			return NULL;
		}
	...
	*q = 0;
	...
	if(*p == '\'' || *p == '"') {
		...
		while(*p && *p != quote) {
			...
			if(q - n->value >= AGFS_BTREE_MAX_KEY_LEN - 1) {
				*err = -EINVAL;
				agfs_q_free(n);
				return NULL;
			}
...
static struct agfs_qnode *agfs_q_parse_term(const char **pp, int *err, int depth)
{
	const char *p = agfs_q_skip(*pp);
	struct agfs_qnode *n;
	...
	if(*p == '(') {
		...
		p++;
		n = agfs_q_parse_expr(&p, err, depth + 1);
		if(!n) {
			return NULL;
		}
		p = agfs_q_skip(p);
...
static struct agfs_qnode *agfs_q_parse_and(const char **pp, int *err, int depth)
{
	struct agfs_qnode *n, *r;

	n = agfs_q_parse_term(pp, err, depth);
	if(!n) {
		return NULL;
	}
	for(;;) {
		...
		if(p[0] == '&' && p[1] == '&') {
			...
			p += 2;
			*pp = p;
			if(!(r = agfs_q_parse_term(pp, err, depth))) {
				agfs_q_free(n);
				return NULL;
			}
			if(!(a = agfs_q_new(BQ_AND))) {
				*err = -ENOMEM;
				agfs_q_free(n);
				agfs_q_free(r);
				return NULL;
...
static struct agfs_qnode *agfs_q_parse_expr(const char **pp, int *err, int depth)
{
	...
	n = agfs_q_parse_and(pp, err, depth);
	if(!n) {
		return NULL;
	}
	for(;;) {
		...
		if(p[0] == '|' && p[1] == '|') {
			...
			p += 2;
			*pp = p;
			if(!(r = agfs_q_parse_and(pp, err, depth))) {
				agfs_q_free(n);
				return NULL;
			}
			if(!(a = agfs_q_new(BQ_OR))) {
				*err = -ENOMEM;
				agfs_q_free(n);
				agfs_q_free(r);
				return NULL;
...
static int agfs_q_eval(struct superblock *sb, struct agfs_qnode *n,
		      struct agfs_qset *out)
{
	struct agfs_qset s1, s2;
	int res;

	if(n->type == BQ_FORMULA) {
		return agfs_q_eval_formula(sb, n, out);
	}
	if((res = agfs_q_eval(sb, n->l, &s1)) < 0) {
		return res;
	}
	if((res = agfs_q_eval(sb, n->r, &s2)) < 0) {
		agfs_qset_free(&s1);
		return res;
	}
	if(n->type == BQ_AND) {
 ...
 */
int agfs_query(struct superblock *sb, const char *q, __u32 *inos, __u32 cap)
{
	const char *p = q;
	struct agfs_qnode *tree;
	struct agfs_qset set;
	int err, res;
	__u32 n;

	/* the value of an empty query is the empty set */
	if(!q || !*q || !agfs_q_skip(q)[0]) {
		return 0;
	}
	tree = agfs_q_parse_expr(&p, &err, 0);
	if(!tree) {
		return err;
	}
	p = agfs_q_skip(p);
	...
	if((res = agfs_q_eval(sb, tree, &set)) < 0) {
		agfs_q_free(tree);
		return res;
	}
	agfs_q_free(tree);
