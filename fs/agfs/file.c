 ...
 * fnx/fs/agfs/file.c
 *
 * AGFS file operations: write (with block allocation via bmap
...
__loff_t agfs_file_llseek(struct inode *i, __loff_t offset)
{
	return offset;
}
 ...
 * and the attribute walkers stop before the content. AGFS_INLINE_MAX
 * (512) fits for every name length up to the 255-byte limit.
 */
int agfs_inline_base(struct inode *i)
{
	int left = i->sb->s_blocksize - (int)sizeof(struct agfs_inode);
	unsigned char *p = i->u.agfs.small_data;
	int pos = 0;
 ...
 */
int agfs_inline_expand(struct inode *i)
{
	__u64 S = i->i_size;
	__blk_t block;
	unsigned int boffset, bytes;
	int blksize = i->sb->s_blocksize;
	unsigned char *data = i->u.agfs.small_data + agfs_inline_base(i);
	__u64 done = 0;
	struct buffer *buf;
...
int agfs_file_read(struct inode *i, struct fd *f, char *buffer,
		   __size_t count)
{
	__size_t n;
	...
	memcpy_b(buffer, i->u.agfs.small_data + agfs_inline_base(i) + f->offset, n);
	f->offset += n;
	return n;
}

/* X-SSD6: write into an inline file's tail (offset+count already known
 * to fit AGFS_INLINE_MAX). Updates the same bookkeeping as the stream
 * write path so the index + times stay consistent. Returns count. */
static __size_t agfs_inline_write(struct inode *i, struct fd *f,
				  ...
				  __u64 old_mtime)
{
	...
	int base = agfs_inline_base(i);

	if((__u64)offset > (__u64)i->i_size) {
		/* sparse write into an inline file: zero the gap */
		memset_b(i->u.agfs.small_data + base + i->i_size, 0,
			offset - i->i_size);
	}
	memcpy_b(i->u.agfs.small_data + base + offset, buffer, count);
	/* zero the tail beyond the new size (a shrinking overwrite can
	 * leave stale bytes past the end) */
	if((__u64)base + newsize < AGFS_INLINE_MAX + 0) {
		int tail = i->sb->s_blocksize - (int)sizeof(struct agfs_inode);
		memset_b(i->u.agfs.small_data + base + newsize, 0,
...
int agfs_file_write(struct inode *i, struct fd *f, const char *buffer,
		   __size_t count)
{
	__blk_t block;
	...
	old_size = i->i_size;
	old_mtime = i->u.agfs.raw.last_modified_time;
	...
	if(S_ISREG(i->i_mode) && count) {
		if(i->u.agfs.raw.flags & AGFS_INODE_INLINE_DATA) {
			if((__u64)(offset + count) <= AGFS_INLINE_MAX) {
				retval = agfs_inline_write(i, f, buffer, count,
							   offset, old_size,
							   old_mtime);
				inode_unlock(i);
				return retval;
			...
			if((retval = agfs_inline_expand(i)) < 0) {
				inode_unlock(i);
				return retval;
			}
		} else if(i->i_size == 0 && offset == 0
			  && count <= AGFS_INLINE_MAX) {
	...
	if(!retval) {
		...
		/* track the stream size so agfs_ifree() truncates (frees the
